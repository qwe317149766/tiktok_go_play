#!/usr/bin/env python3
"""
补全 startup_cookie_accounts 表的 device_create_time 字段

从 account_json 中的 create_time 字段提取注册时间戳，并更新到 device_create_time 字段。
支持多种时间格式：
- "2006-01-02 15:04:05"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="补全 startup_cookie_accounts 表的 device_create_time 字段")
    ...
    try:
        with conn.cursor() as cur:
            ...
            query = f"SELECT id, account_json FROM `{table}` WHERE device_create_time IS NULL"
            if args.limit > 0:
            ...
            for i, (record_id, account_json_str) in enumerate(rows):
                try:
                    account_json = json.loads(account_json_str)
                    create_time_val = account_json.get("create_time")
                    ...
                    failed += 1
                    print(f"[warn] 记录 id={record_id} 的 account_json 解析失败: {e}", file=sys.stderr)
                except Exception as e:
                    failed += 1
