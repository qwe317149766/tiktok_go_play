...
var (
	config = Config{
		...
		TargetSuccess:  1000000,
		MaxRequests:    1900000,
		AwemeID:        "7587841512851967252",
		ResultFile:     "results.jsonl",
		ErrorFile:      "error.log",
	}
	cacheFile = "device_cache.txt" // 设备缓存文件
	// IP切换配置：每个IP使用多少次后切换（默认100次）
	maxUsePerIP int64 = 100
)
...
func NewEngine() (*Engine, error) {
	...
	// 设置每个IP的最大使用次数（默认100次）
	if proxyManager := GetProxyManager(); proxyManager != nil {
		proxyManager.SetMaxUsePerIP(maxUsePerIP)
	}
...
func executeTask(taskID int, awemeID, deviceJSON, proxy string) (bool, map[string]interface{}) {
	...
	// 记录客户端使用次数（无论成功或失败都记录）
	if proxyManager != nil {
		proxyManager.RecordClientUse(proxy)
	}
	if err != nil {
		// 判断是否是网络错误
		errStr := err.Error()
