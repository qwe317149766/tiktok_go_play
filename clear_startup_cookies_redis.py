...
def _normalize_pool_base(prefix: str) -> str:
    """
    若传入形如 "tiktok:startup_cookie_pool:3"，则回退为 "tiktok:startup_cookie_pool"
    """
...
def _keys_for_prefix(prefix: str) -> List[str]:
    # 与 Go 的 startup_cookie_pool 一致：仅 ids/data/use
    return [
        f"{prefix}:ids",
        f"{prefix}:data",
        f"{prefix}:use",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="清空 signup 注册成功写入的 Redis startup_cookie_pool（ids/data/use）。"
    )
    parser.add_argument(
        "--prefix",
        default="",
        help="cookies 池 base prefix（默认读取 REDIS_STARTUP_COOKIE_POOL_KEY，最后 tiktok:startup_cookie_pool）",
    )
    parser.add_argument(
        "--shards",
        type=int,
        default=0,
        help="分库数量（默认读取 REDIS_COOKIE_POOL_SHARDS；<=1 表示不分库）",
    )
    parser.add_argument(
        "--password",
        default="",
    ...
    if not prefix:
        prefix = (os.getenv("REDIS_STARTUP_COOKIE_POOL_KEY") or "").strip()
    if not prefix:
        prefix = "tiktok:startup_cookie_pool"

    # 避免用户传了 :idx 只清一个 shard：这里统一按 base 来清
    base_prefix = _normalize_pool_base(prefix)

    shards = int(args.shards or 0)
    if shards <= 0:
        shards = int(os.getenv("REDIS_COOKIE_POOL_SHARDS", "1") or "1")
    if shards <= 0:
        shards = 1
    ...
    print(f"[cookie_pool] base_prefix={base_prefix} shards={shards} prefixes={prefixes}")
